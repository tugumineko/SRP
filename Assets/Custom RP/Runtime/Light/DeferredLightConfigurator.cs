...
public class DeferredLightConfigurator
{
    ...
    private ComputeBuffer _lightPositionAndRangeBuffer = new ComputeBuffer(MAX_LIGHT_COUNT, sizeof(float) * 4);
    private ComputeBuffer _lightColorsBuffer = new ComputeBuffer(MAX_LIGHT_COUNT, sizeof(float) * 4);
        ...
        foreach (var l in visibleLights)
        {
            switch (l.lightType)
            {
                case LightType.Directional:
                    ...
                    else if (LightUtil.CompareDirectionalLight(l.light, visibleLights[mainLightIndex].light) < 0)
                    {
                        mainLightIndex = lightIndex;
                    }
    ...
    public struct LightData
    {
        ...
        public bool HasMainLight => mainLightIndex >= 0;
    }
}
